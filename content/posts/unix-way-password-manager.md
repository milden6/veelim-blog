...
Не буду расписывать о правилах создания и хранения паролей так, как это дело каждого. Если вы считаете, что использовать один пароль везде это хорошо или вам нечего скрывать, то это, конечно, плохо, но это ваше право.

Если все таки нужен хороший менеджер паролей с открытым исходным кодом, но без лишних сложностей, могу порекомендовать [Bitwarden](https://bitwarden.com/) и [KeePassXC](https://keepassxc.org/).

**Bitwarden** - это самый удобный вариант в плане синхронизации. Пароли хранятся на сервере компании Bitwarden, Inc., но в [хешированном виде](https://bitwarden.com/blog/vault-security-bitwarden-password-manager/), даже если база будет взломана, то пароли будет не так просто заполучить. Бесплатный план без ограничений на количество хранимых паролей. Если не доверяете Bitwarden, Inc. можно развернуть свой сервер. Есть 2 реализации: [официальная](https://github.com/bitwarden/server) на C#, но очень требовательная к ресурсам и [неофициальная](https://github.com/dani-garcia/vaultwarden), написанная на Rust с минимальными требованиями. Я сам использую Bitwarden и мне он нравится.
...
3. **Почему не QtPass или Pass4Win на Windows?**
   Они мне не нравятся, не испытываю неудобств при использовании Termux'a.
